...
##' 
##' @title Return the latest N weibos metioned the authenticating user. (That is the weibo contained @@username) 
...
##' @param filter_by_source Return the weibos by the source type. 0 : all types; 1 : from weibo; 2 : from weiqun; The default value is 0.
##' @param filter_by_type Return the weibos by the type. 0 : all types; 1 : original weibos; The default value is 0.
...
##' @references \url{http://open.weibo.com/wiki/2/statuses/mentions}
##' @keywords Weibo
##' @examples \dontrun{
##' 
...
statuses.mentions <- function(roauth, count = 20, page = 1, filter_by_author = 0, 
		filter_by_source = 0, filter_by_type = 0, ...) {
	requestURL <- "https://api.weibo.com/2/statuses/mentions.json"
	funCall <- match.call()
