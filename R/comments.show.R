...
##' 
##' @title Return comments of one weibo by the weibo ID.
##' @param roauth A OAuth object created by \code{\link{createOAuth}}.
##' @param id The weibo ID to be queried.
...
##' @references \url{http://open.weibo.com/wiki/2/comments/show}
##' @keywords Comments
##' @examples \dontrun{
##' 
...
comments.show <- function(roauth, id, count = 20, page = 1, filter_by_author = 0, ...) {
	requestURL <- "https://api.weibo.com/2/comments/show.json"
	funCall <- match.call()
