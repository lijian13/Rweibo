...
##' Return the latest weibos of the users that are following the authenticating user and are being followed by the authenticating user.
##' 
##' @title Return the latest weibos of the users that are following the authenticating user and are being followed by the authenticating user.
...
##' @references \url{http://open.weibo.com/wiki/2/statuses/bilateral_timeline}
##' @keywords Weibo
##' @examples \dontrun{
##' 
##' statuses.bilateral_timeline(roauth, count = 5)
##' }

statuses.bilateral_timeline <- function(roauth, count = 20, page = 1, base_app = 0, feature = 0, ...) {
	requestURL <- "https://api.weibo.com/2/statuses/bilateral_timeline.json"
	funCall <- match.call()
