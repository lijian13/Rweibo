...
##' Return the latest comments metioned the authenticating user.(That is the comment contained @@username)
##' 
##' @title Return the latest comments metioned the authenticating user.
...
##' @references \url{http://open.weibo.com/wiki/2/comments/mentions}
##' @keywords Comments
##' @examples \dontrun{
##' 
##' comments.mentions(roauth, count = 5)
##' }

comments.mentions <- function(roauth, count = 20, page = 1, filter_by_author = 0, 
		filter_by_source = 0, ...) {
	requestURL <- "https://api.weibo.com/2/comments/mentions.json"
	funCall <- match.call()
