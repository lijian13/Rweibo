...
##' 
##' @title Return the friends list of a user.
...
##' @references \url{http://open.weibo.com/wiki/2/friendships/friends}
...
friendships.friends <- function(roauth, uid, screen_name, count = 20, cursor = 0, ...) {
	requestURL <- "https://api.weibo.com/2/friendships/friends.json"
	funCall <- match.call()
