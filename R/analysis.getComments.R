...
##' 
##' @title Return the data.frame of comments of a original weibo.
...
analysis.getComments <- function(roauth, mid, count = -1, startcount = 1) {
	...
	if (count <= 0) count <- weiboList$comments_count
	if (startcount <= 0) stop("'startcount' should be gerater than 0!")
	counttotal <- count + startcount - 1
	counttotal <- min(counttotal, weiboList$comments_count)
