...
##' 
##' @title Return the followers list of a user.
##' @param roauth A OAuth object created by \code{\link{createOAuth}}.
##' @param uid User ID to be queried.
##' @param screen_name User nickname to be queried.
##' @param count The returned count for every page.The default value is 20. The max value is 200.
##' @param cursor The cursor of the returned data.The returned next_cursor is for the next page.The returned previous_cursor is for the previous page. Default is 0.
##' @param ... Other request parameters for this API.
##' @note You must provide a parameter either uid or screen_name. You can get 5000 followers at the most.
##' @return 
...
##' @references \url{http://open.weibo.com/wiki/2/friendships/followers}
...
friendships.followers <- function(roauth, uid, screen_name, count = 20, cursor = 0, ...) {
	requestURL <- "https://api.weibo.com/2/friendships/followers.json"
	funCall <- match.call()
