...
##' Return the users that are being followed by the authenticating user and are following the specified user.
##' 
##' @title Return the users that are being followed by the authenticating user and are following the specified user.
##' @param roauth A OAuth object created by \code{\link{createOAuth}}.
##' @param uid User ID to be specified.
...
##' @references \url{http://open.weibo.com/wiki/2/friendships/friends_chain/followers}
##' @keywords Comments
##' @examples \dontrun{
##' 
...
friendships.friends_chain.followers <- function(roauth, uid, count = 20, page = 1, ...) {
	requestURL <- "https://api.weibo.com/2/friendships/friends_chain/followers.json"
	funCall <- match.call()
